...
class GeneralRecommender(AbstractRecommender):
    ...
    def __init__(self, config, dataloader):
        ...
        if not config['end2end'] and config['is_multimodal_model']:
            ...
            print(config['vision_feature_file'])
            print(config['text_feature_file'])

            if config['vision_feature_file'] is not None:
                print(f"VISION FEATURES: {config['vision_feature_file']}")
                ...
                if os.path.isfile(v_feat_file_path):
                    ...
                    print(f'Visual features shape: {self.v_feat.shape}')
                else:
                    print(f'{v_feat_file_path} does not exists!')

            if config['text_feature_file'] is not None:
                print(f"TEXT FEATURES: {config['text_feature_file']}")
                ...
                if os.path.isfile(t_feat_file_path):
                    ...
                    print(f'Textual features shape: {self.t_feat.shape}')
                else:
                    print(f'{t_feat_file_path} does not exists!')
