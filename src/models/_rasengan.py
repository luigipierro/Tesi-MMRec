...
class RASENGAN(GeneralRecommender):
    def __init__(self, config, dataset):
        ...
        if self.pt_modality == 'autoencoder':
            self.ae_weight = config['ae_weight']
        ...
        # init user and item embeddings with autoencoders data or random
        if self.pt_modality == 'autoencoder':

            # define autoencoders for user and item embeddings
            self.user_autoencoder = AutoEncoder(self.user_review_embs.shape[1], self.hidden_dim).to(self.device)
            self.item_autoencoder = AutoEncoder(self.item_review_embs.shape[1], self.hidden_dim).to(self.device)
            
            # init with compressed representations from autoencoder
            with torch.no_grad():
                user_encoded, _ = self.user_autoencoder(self.user_review_embs)
                item_encoded, _ = self.item_autoencoder(self.item_review_embs)
                self.user_embeddings.weight.data.copy_(user_encoded)
                self.item_embeddings.weight.data.copy_(item_encoded)

            # define the reconstruction loss
            self.recon_loss = nn.MSELoss()

        elif self.pt_modality == 'linear':
            
            self.user_pt_linear = nn.Sequential(
                nn.Linear(self.user_review_embs.shape[1], self.hidden_dim),
                nn.ReLU(0.2))
            self.item_pt_linear = nn.Sequential(
                nn.Linear(self.item_review_embs.shape[1], self.hidden_dim),
                nn.ReLU(0.2),)
            nn.init.xavier_uniform_(self.user_pt_linear[0].weight)
            nn.init.xavier_uniform_(self.item_pt_linear[0].weight)
            # nn.init.xavier_uniform_(self.user_embeddings.weight)
            # nn.init.xavier_uniform_(self.item_embeddings.weight)

        elif self.pt_modality == 'concat':

            # ID user and item embeddings
            nn.init.xavier_uniform_(self.user_embeddings.weight)
            nn.init.xavier_uniform_(self.item_embeddings.weight)

            # PT user and item embeddings
            self.user_pt_linear = nn.Sequential(
                nn.Linear(self.user_review_embs.shape[1], self.hidden_dim),
                nn.ReLU(0.2))
            self.item_pt_linear = nn.Sequential(
            ...
            # define concat layer
            self.user_concat = nn.Sequential(
                nn.Linear(self.hidden_dim*2, self.hidden_dim),
                nn.ReLU(0.2))
            self.item_concat = nn.Sequential(
                nn.Linear(self.hidden_dim*2, self.hidden_dim),
                nn.ReLU(0.2))
            ...
            # init by default, but if 'autoencoder' or 'linear' will change
    ...
    def autoencode_embeddings(self):
        # Get encoded representations
        user_encoded, user_decoded = self.user_autoencoder(self.user_review_embs)
        item_encoded, item_decoded = self.item_autoencoder(self.item_review_embs)
        
        # Calculate reconstruction loss
        user_recon_loss = self.recon_loss(user_decoded, self.user_review_embs)
        item_recon_loss = self.recon_loss(item_decoded, self.item_review_embs)
        
        # Total autoencoder loss
        ae_loss = user_recon_loss + item_recon_loss
        
        return user_encoded, item_encoded, ae_loss
    
    def forward(self):

        if self.pt_modality == 'autoencoder':
            user_emb, item_emb, _ = self.autoencode_embeddings()

        elif self.pt_modality == 'linear':
            user_emb = self.user_pt_linear(self.user_review_embs)
            item_emb = self.item_pt_linear(self.item_review_embs)

        elif self.pt_modality == 'concat':
            user_emb = self.user_concat(torch.cat([self.user_embeddings.weight, self.user_pt_linear(self.user_review_embs)], dim=1))
            item_emb = self.item_concat(torch.cat([self.item_embeddings.weight, self.item_pt_linear(self.item_review_embs)], dim=1))
        
        elif self.pt_modality == 'id':
            # use only ID embeddings - no pre-trained embeddings
            user_emb, item_emb = self.user_embeddings.weight, self.item_embeddings.weight
        else:
            raise ValueError("Unsupported pt_modality: {}".format(self.pt_modality))
