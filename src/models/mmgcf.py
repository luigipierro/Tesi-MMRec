...
Late-fusion of LightGCN ID embeddings with multimodal (image/text) item features.

Fusion modes  : mean | sum | prod | concat
Weighting modes: equal | alpha | normalized

Adapted for the MMRec framework following FREEDOM conventions.
...
class MMGCF(GeneralRecommender):
    def __init__(self, config, dataset):
        ...
        self.fusion_mode    = config['fusion_mode']   # mean | sum | prod | concat
        ...
        # Raw pretrained features are stored in (optionally trainable) Embeddings;
        # a learnable Linear projects them to feat_embed_dim on every forward pass.
        self.n_modalities = 0
    ...
    def _apply_fusion(self, tensors, concat_layer=None):
        ...
        mean / sum / prod — element-wise ops; all tensors must share shape.
